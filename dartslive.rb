...
def get_pages(number)
    ...
    pages += (number % COUNTPERPAGE) > 0 ? 1 : 0
...
prefs.each { |pref|
    dlive.Go("/list/"+ pref + "?dl2=1")

    count = dlive.GetItem(".number")[0].inner_text.to_i

    i = 0
    while i < get_pages(count) do
        dlive.Go("/list/"+ pref + "?dl2=1&p=" + i.to_s)
        list.concat(dlive.RetrieveList("div.searchList2", method(:proc_split_shoptable_to_array)))
        i += 1
    end
}
